...
/// Minimum account balanced. No account's free balance should fall below this, unless if the
...
pub enum SystemCall {
	/// Do nothing.
	///
	/// This will only ensure that `data` is remarked in the block data.
...
///
/// If an account has no reserved balance, and its free balance falls to zero, it is considered
/// destroyed.
