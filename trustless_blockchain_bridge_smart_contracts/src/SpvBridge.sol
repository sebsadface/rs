/// THIS CONTRCT SHOULD NOT BE GRADED.
...
contract SpvBridge {
    ...
    function submit_new_header(Header calldata header) external payable {
        if msg.value < relay_fee {
            revert("Insufficient relay fee");
        }

        if header_is_known(hash_header(header)) {
            revert("Header already known");
        }

        parent_height = headers[header.parent].height;
        if parent_height + 1 != header.height {
            revert("Header height is not one more than parent");
        }

        if hash_header(header) > difficulty_threshold {
            revert("Header does not satisfy PoW threshold");
        }

        if header.parent != canon_chain[parent_height] {
            revert("Header does not extend canon chain");
        }
